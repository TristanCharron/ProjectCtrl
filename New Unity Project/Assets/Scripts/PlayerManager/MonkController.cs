...
public class MonkController : MonoBehaviour
{
    ...
    bool isHitValid()
    {
        return currentTeam.TeamID != ballManager.PossessedTeam && ballManager.PossessedTeam != teamManager.teamID.Neutral;
    }
    ...
    public void onPush()
    {
        if (pulledVelocity != 0)
        {
            ...
            ballManager.onPush(Quaternion.LookRotation(LookAtTransform.position - cursorTransform.transform.position) * -transform.up, this);

        ballManager.onChangeTeamPossession(currentTeam.TeamID);
