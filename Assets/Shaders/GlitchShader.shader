...
#include "UnityCG.cginc"
	...
	half4 frag(v2f i) : COLOR
	{
		...
		i.uv.xy += (_Chroma > 1) ? (normal.xy - 0.5) * displace * _Intensity 
			: - (normal.xy - 0.5) * displace * _Intensity;



		half4 color = tex2D(_MainTex,  i.uv.xy);
		half4 chroma = tex2D(_MainTex, _Chroma > 1 ? i.uv.xy - 0.01 * filterRadius * _Intensity
			: i.uv.xy + 0.01 * filterRadius * _Intensity);
